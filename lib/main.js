...
var t4tWelcomePage = self.data.url("welcome.html");
var t4tPanel = self.data.url("panel.html");
...
var myScript = self.data.url("myScript.js");

var panel = require("panel").Panel({
    width: 250,
    height: 80,
    contentURL: t4tPanel,
    contentScriptFile: myScript
})
var widget = widgets.Widget({
    id: "t4t-progress",
    label: "Checking T4T stats",
    contentURL: t4tIconURL,
    panel: panel
});
...
// Update panel message with user Preferences
panel.port.emit('interval', prefs.PollInterval);
    ...
    if (ss.storage.userStats.n_events < recentUserStats.n_events) {
        ...
        if (ss.storage.userStats.n_events != 0) {
            ...
            if (n_events == 1) {
              ...
              var msg = "Congratulations! You have simulated: " + siprefix(n_events) + " new events";
            }
            notifications.notify({
    ...
    if (ss.storage.userStats.n_jobs < recentUserStats.n_jobs) {
        ...
        if (ss.storage.userStats.n_jobs != 0) {
            ...
            if (n_jobs == 1) {
              ...
              var msg = "Congratulations! You have completed: " + siprefix(n_jobs) + " new jobs";
            }
        }
        else {
    ...
    if (ss.storage.userStats.n_hosts < recentUserStats.n_hosts) {
        ...
        if (ss.storage.userStats.n_hosts != 0) {
            ...
            if (n_hosts == 1) {
              ...
              var msg = "Thanks a lot! You have added: " + siprefix(n_hosts) + " new computers";
            }
        }
        else {
            var title = "Computers";
